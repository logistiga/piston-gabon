...
CREATE POLICY "payments_crud_policy_v2" ON payments
  USING (true)
  WITH CHECK (true);

CREATE POLICY "cash_register_crud_policy_v2" ON cash_register
  USING (true)
  WITH CHECK (true);
...
DROP TRIGGER IF EXISTS update_purchase_order_payment_status_trigger ON payments;

-- Create separate triggers for each operation
CREATE TRIGGER update_purchase_order_payment_status_insert_trigger
  AFTER INSERT ON payments
  FOR EACH ROW
  WHEN (NEW.entity_type = 'purchase_order')
  EXECUTE FUNCTION update_purchase_order_payment_status();

CREATE TRIGGER update_purchase_order_payment_status_update_trigger
  AFTER UPDATE ON payments
  FOR EACH ROW
  WHEN (NEW.entity_type = 'purchase_order')
  EXECUTE FUNCTION update_purchase_order_payment_status();

CREATE TRIGGER update_purchase_order_payment_status_delete_trigger

...
DO $$ BEGIN
  DROP TRIGGER IF EXISTS set_account_number_trigger_v3 ON client_accounts;
  DROP TRIGGER IF EXISTS update_account_balance_trigger_v3 ON client_account_transactions;
  DROP TRIGGER IF EXISTS track_account_changes_trigger_v3 ON client_accounts;
END $$;

-- Drop existing functions if they exist
DO $$ BEGIN
  DROP FUNCTION IF EXISTS set_account_number_v3();
  DROP FUNCTION IF EXISTS update_account_balance_v3();
  DROP FUNCTION IF EXISTS track_account_changes_v3();
END $$;
...
BEGIN
  ...
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON client_accounts', pol.policyname);
  END LOOP;
END $$;

-- Create client_accounts table if it doesn't exist
...
-- Create client_account_transactions table if it doesn't exist
...
-- Create client_account_audit table if it doesn't exist
...
CREATE POLICY "policy_admin_manage_accounts_v3"
ON client_accounts
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "policy_staff_view_accounts_v3"
ON client_accounts
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "policy_client_view_own_account_v3"
ON client_accounts
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "policy_admin_manage_transactions_v3"
...
CREATE POLICY "policy_staff_view_transactions_v3"
ON client_account_transactions
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "policy_client_view_own_transactions_v3"
ON client_account_transactions
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "policy_admin_view_audit_v3"
...
CREATE OR REPLACE FUNCTION generate_account_number_v3()
RETURNS text AS $$
DECLARE
  new_number text;
  counter integer := 1;
...
CREATE OR REPLACE FUNCTION set_account_number_v3()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER set_account_number_trigger_v3
  BEFORE INSERT ON client_accounts
  FOR EACH ROW
  EXECUTE FUNCTION set_account_number_v3();
...
CREATE INDEX IF NOT EXISTS idx_client_accounts_auth_user_id ON client_accounts(auth_user_id);
CREATE INDEX IF NOT EXISTS idx_client_accounts_status ON client_accounts(status);
CREATE INDEX IF NOT EXISTS idx_client_accounts_account_number ON client_accounts(account_number);

CREATE INDEX IF NOT EXISTS idx_client_account_transactions_account_id ON client_account_transactions(account_id);
CREATE INDEX IF NOT EXISTS idx_client_account_transactions_type ON client_account_transactions(type);
CREATE INDEX IF NOT EXISTS idx_client_account_transactions_transaction_date ON client_account_transactions(transaction_date);

CREATE INDEX IF NOT EXISTS idx_client_account_audit_account_id ON client_account_audit(account_id);
CREATE INDEX IF NOT EXISTS idx_client_account_audit_action ON client_account_audit(action);
CREATE INDEX IF NOT EXISTS idx_client_account_audit_performed_at ON client_account_audit(performed_at);

...
CREATE POLICY "admin_settings_policy_v2"
ON settings
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "read_settings_policy_v2"
ON settings
FOR SELECT
TO authenticated
USING (true);
...
COMMENT ON POLICY "admin_settings_policy_v2" ON settings IS 'Allows administrators full access to settings';
COMMENT ON POLICY "read_settings_policy_v2" ON settings IS 'Allows all authenticated users to read settings';
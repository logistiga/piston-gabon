...
CREATE POLICY "policy_view_user_profiles_v6"
ON user_profiles
FOR SELECT 
TO authenticated
USING (
...
CREATE POLICY "policy_update_own_profile_v6"
ON user_profiles
FOR UPDATE
TO authenticated
...
CREATE POLICY "policy_admin_manage_profiles_v6"
ON user_profiles
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "policy_admin_manage_settings_v6"
ON settings
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "policy_read_settings_v6"
ON settings
FOR SELECT
TO authenticated
USING (true);
...
COMMENT ON POLICY "policy_view_user_profiles_v6" ON user_profiles IS 'Allows users to view their own profile and admins to view all';
COMMENT ON POLICY "policy_update_own_profile_v6" ON user_profiles IS 'Allows users to update their own profile';
COMMENT ON POLICY "policy_admin_manage_profiles_v6" ON user_profiles IS 'Allows administrators to manage all profiles';

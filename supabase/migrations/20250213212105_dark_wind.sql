...
SELECT 
  ...
  END as type,
  CASE
    -- Pour les paiements, le montant dépend du type d'entité
    WHEN bt.type = 'payment' THEN
      CASE
        -- Les paiements de tickets et factures sont des entrées positives
        WHEN pi.entity_type IN ('ticket', 'invoice') THEN ABS(bt.amount)
        -- Les paiements de commandes sont des sorties négatives
        WHEN pi.entity_type = 'purchase_order' THEN -ABS(bt.amount)
        ELSE bt.amount
      END
...
COMMENT ON COLUMN bank_transactions_details.amount IS 'Transaction amount (positive for income, negative for expenses)';

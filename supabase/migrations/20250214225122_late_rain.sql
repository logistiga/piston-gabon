...
BEGIN
  ...
  IF NEW.payment_method IN ('check', 'bank_transfer') AND NEW.bank_id IS NOT NULL THEN
    ...
    INSERT INTO bank_transactions (
      id,  -- Use same ID as payment for linking
      bank_id,
      type,
      amount,
      reference,
...
COMMENT ON FUNCTION create_bank_transaction_from_payment IS 'Creates a bank transaction when a payment is made using bank payment methods';
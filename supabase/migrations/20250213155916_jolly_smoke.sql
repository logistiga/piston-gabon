...
DROP POLICY IF EXISTS "policy_read_company_settings" ON company_settings;
DROP POLICY IF EXISTS "policy_update_company_settings" ON company_settings;

-- Create policies for company_settings
CREATE POLICY "policy_read_company_settings_v2"
ON company_settings
FOR SELECT
TO authenticated
USING (true);

CREATE POLICY "policy_update_company_settings_v2"
ON company_settings
FOR UPDATE
TO authenticated
USING (
...
CREATE POLICY "policy_insert_company_settings_v2"
ON company_settings
FOR INSERT
TO authenticated
WITH CHECK (
...
COMMENT ON POLICY "policy_read_company_settings_v2" ON company_settings IS 'Allow all authenticated users to read company settings';
COMMENT ON POLICY "policy_update_company_settings_v2" ON company_settings IS 'Allow only administrators to update company settings';
COMMENT ON POLICY "policy_insert_company_settings_v2" ON company_settings IS 'Allow only administrators to insert company settings';

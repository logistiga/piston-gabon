...
CREATE POLICY "admin_company_settings_policy_v4"
ON company_settings
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "read_company_settings_policy_v4"
ON company_settings
FOR SELECT
TO authenticated
USING (true);
...
COMMENT ON POLICY "admin_company_settings_policy_v4" ON company_settings IS 'Allows administrators full access to company settings';
COMMENT ON POLICY "read_company_settings_policy_v4" ON company_settings IS 'Allows all authenticated users to read company settings';
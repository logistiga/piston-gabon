...
SELECT 
  ...
  CASE
    ...
    WHEN bt.type = 'payment' THEN
      CASE payment_info.document_type
        -- Les tickets et factures sont des entrées positives
        WHEN 'ticket' THEN ABS(bt.amount)
        WHEN 'invoice' THEN ABS(bt.amount)
        -- Les commandes sont des sorties négatives
        WHEN 'purchase_order' THEN -ABS(bt.amount)
        ELSE bt.amount
      END

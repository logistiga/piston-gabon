...
-- Add missing bank transactions for existing payments
...
SELECT 
  ...
  CASE 
    WHEN p.entity_type = 'ticket' THEN 
      'Paiement ticket ' || t.reference || ' - ' || t.client_nom
    WHEN p.entity_type = 'invoice' THEN
      'Paiement facture ' || i.reference || ' - ' || i.client_nom
  END as description,
  p.reference,
  p.payment_date,
  'confirmed'::text as status,
  'TR_' || LPAD(CAST(FLOOR(random() * 1000000) AS text), 6, '0') as transaction_number
FROM payments p
